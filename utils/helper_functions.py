...
def preprocessing(list_of_tokens, head_indexes_2d):
    ...
    tokens_x_2d = [[] for _ in range(batch_size)]
    selected_pos = [[] for _ in range(batch_size)]  # Add this to store POS tags, mock-up for now
    selected_entity = [[] for _ in range(batch_size)]  # Add this to store entity information, mock-up for now
    attention_mask = [[] for _ in range(batch_size)]  # Attention mask, mock-up for now

    # Dummy logic for filling selected_pos, selected_entity, and attention_mask
    for i in range(batch_size):
        tokens_x_2d[i] = list_of_tokens[i]
        selected_pos[i] = [0] * len(list_of_tokens[i])  # Mock-up POS tags (0 as placeholder)
        selected_entity[i] = [0] * len(list_of_tokens[i])  # Mock-up entities (0 as placeholder)
        attention_mask[i] = [1] * len(list_of_tokens[i])  # Mock-up attention mask (all 1s for simplicity)
    
    return tokens_x_2d, selected_pos, selected_entity, attention_mask
