...
from models.graph_convolution import GCN
...
class EventExtractor(nn.Module):
    ...
    def __init__(
        ...
        config_class, model_class, tokenizer_class, model_name_or_path = MODEL_CLASSES[
        ...
        self.postag_embed = nn.Embedding(
            num_embeddings=postag_size, embedding_dim=args.postag_embedding_dim
        )

        self.deptags_embed = nn.Embedding(
            num_embeddings=deptags_size, embedding_dim=args.deptags_embedding_dim
        )

        self.trigger_embed = nn.Embedding(
            num_embeddings=trigger_size, embedding_dim=args.trigger_embedding_dim
        )

        self.entity_mention_embed = nn.Embedding(
            num_embeddings=entity_size, embedding_dim=args.entity_embedding_dim
        )
        ...
        self.out_mlp = nn.Sequential(
            nn.Linear(args.GCN_hidden_dim * 3, args.GCN_hidden_dim), nn.ReLU()
        )
        ## *3 because appending together trigger_ouputs and entity_outputs

        self.fc_argument_roles_gcn = nn.Sequential(
            nn.Linear(args.GCN_hidden_dim, argument_size)
        )

        self.fc = nn.Sequential(nn.Linear(self.model_input_size, event_size))

        self.fc_entity_mention = nn.Sequential(
            nn.Linear(self.model_embedding_dim, entity_size)
        )
    ...
    def encode_tokens(self, selected_tokens, attention_mask):
        ...
        outputs = self.transformer(
            input_ids=selected_tokens, attention_mask=attention_mask
        )
        word_enc = outputs[0]  ### the last hidden state

        return word_enc

    def build_trigger_entity(
        self, encoded_tokens, selected_pos, selected_entity, event_hat_2d, arguments_2d
    ):
        ...
        for i in range(batch_size):
            predicted_triggers = find_triggers(
                [idx2trigger_tag[trigger] for trigger in event_hat_2d[i].tolist()]
            )
            actual_sent_len = 0
            ...
            if (
                len(predicted_triggers) > 5
            ):  ## prune away number of predicted trigger that is more than 5
                ## assumption: each sentence has less than 5 events
                continue

            candidates = arguments_2d[i]["candidates"]
            for candidate in candidates:
                argument_tensor = torch.zeros(
                    [argument_window_size, self.model_input_size]
                )
                e_start, e_end, e_type_str = candidate
                j = 0

                for idx in range(e_start, e_end):
                    argument_tensor[j] = argument_xx[
                        i,
                        idx,
                    ]
                    j += 1
            ...
            for predicted_trigger in predicted_triggers:
                ...
                for c, candidate in enumerate(candidates):
                    ...
                    trigger_entity_hidden.append(
                        torch.cat([event_tensor, golden_argument_all[c]], 0)
                    )
                    trigger_entity_keys.append(
                        (i, t_start, t_end, t_type_str, e_start, e_end, e_type_str)
                    )

        return trigger_entity_hidden, trigger_entity_keys

    def classify_argument_roles_gcn(
        self,
        selected_tokens,
        selected_pos,
        selected_entity,
        ...
        input_tensor = torch.cat(
            [selected_tokens, postags_embed, entity_embed], 2
        )  ## torch.Size([2, 54, 868])
        ...
        for (
            ...
            e_end,
            e_type_str,
        ) in trigger_entity_keys:
            a_label = argument2idx["NONE"]
            if (t_start, t_end, t_type_str) in arguments_2d[i][
                "events"
            ]:  ## if predicted trigger exist in Golden-Events
                for a_start, a_end, a_type_idx in arguments_2d[i]["events"][
                    (t_start, t_end, t_type_str)
                ]:
                    if e_start == a_start and e_end == a_end:
                        a_label = a_type_idx
                        break
            trigger_entity_y_1d.append(
                a_label
            )  ## filter out only matched arguments (keep only argument labels)
