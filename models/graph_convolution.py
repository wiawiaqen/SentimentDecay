...
class GCN(nn.Module):
    ...
    def forward(self, adj, gcn_inputs):
        ...
        # gcn layer
        denom = (
            adj.sum(2).unsqueeze(2) + 1
        )  ## [batch_size, maxlen, 1]  - suming up at dimension = 2
        mask = (
            (adj.sum(2) + adj.sum(1)).eq(0).unsqueeze(2)
        )  ## [batch_size, maxlen, 1] - suming up at dimension 1 and 2
